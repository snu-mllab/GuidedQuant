...
template <>
__device__ __forceinline__ void dequant<1, true>(const uint32_t q[1], uint32_t q_w[4]) {
    ...
    #pragma unroll
    for (int i = 0; i < 4; i++)
        q_w[i] = (q_w[i] & 0x0f0f0f0f) | ((q_w[i] & 0xf0f0f0f0) >> 3);
}


template <>
__device__ __forceinline__ void dequant<1, false>(const uint32_t q[1], uint32_t q_w[8]) {
    constexpr uint32_t mask0 = 0x88888888;
    constexpr uint32_t mask1 = 0x44444444;
    constexpr uint32_t mask2 = 0x22222222;
    constexpr uint32_t mask3 = 0x11111111;
...
__device__ __forceinline__ void dequant<2, true>(const uint32_t q[2], uint32_t q_w[4]) {
    ...
    // fast transpose
    q_w[0] = ((q[0]&mask0) >> 2) | ((q[1]&mask0) >> 3);
    q_w[1] = ((q[0]&mask1) >> 1) | ((q[1]&mask1) >> 2);
    q_w[2] = ((q[0]&mask2) >> 0) | ((q[1]&mask2) >> 1);
    q_w[3] = ((q[0]&mask3) << 1) | ((q[1]&mask3) >> 0);
